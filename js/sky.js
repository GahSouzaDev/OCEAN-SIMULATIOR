// js/sky.js — SOL NASCE NO MAR (na frente das cidades) E SE PÕE ATRÁS DA SERRA
...
const SD = { x: 0.40, z: 0.92 };   // mar (leste da baía)
const CC = { x: 0.925, z: -0.383 };// eixo da costa
...
export function initSky() {
  ...
  state.scene.add(sky); state.sky = sky;
  const su = sky.material.uniforms;
  su['turbidity'].value = 4; su['rayleigh'].value = 2.0;
  ...
  boatSpotLight.position.set(0, 8, 0); boatSpotLight.castShadow = false;
  state.scene.add(boatSpotLight); state.scene.add(boatSpotLight.target);
  boatSpotLight.target.position.set(0, 0, 0);
  boatFillLight = new THREE.PointLight(0xffffff, 1.8, 6, 2);
  boatFillLight.position.set(0, 2, 0); boatFillLight.castShadow = false;
  ...
  starGroup = new THREE.Group();
  {
    ...
    starMat = new THREE.PointsMaterial({ size: 1.6, sizeAttenuation: false, color: 0xffe4c8,
...
function sunColorAtHour(hour, out) {
  ...
  else if (hour < 20) out.copy(warmAmber).multiplyScalar(1.0 - (hour - 18.5) / 1.5);
}
...
export function updateSky(cbLights) {
  ...
  // 🌅 NASCENTE NO MAR / POENTE ATRÁS DA SERRA:
  // az=0 (06h) → horizontal = +SD (frente das cidades)
  // az=π (18h) → horizontal = -SD (atrás das cidades, sobre a serra)
  const cosAz = Math.cos(az);
  const drift = Math.sin(az) * 0.30;
  sunDir2.set(
    SD.x * cosAz * hz + CC.x * drift * hz,
    Math.sin(elev),
    SD.z * cosAz * hz + CC.z * drift * hz
  ).normalize();
  ...
  if (boatSpotLight) {
    ...
    boatSpotLight.intensity = 3.5; boatSpotLight.color.setHex(0xffffff);
  }
  if (boatFillLight) {
    boatFillLight.position.copy(state.boatRoot.position);
    boatFillLight.position.y += 2;
    boatFillLight.intensity = 1.8; boatFillLight.color.setHex(0xffffff);
  }
  ...
  const sunMinOpacity = sunSpriteFade * 0.55;
